...
/** Insert the card row itself and return it. */
...
/**
 * Insert attempt for an autogenerated candidate code.
 *
 * Resolves `null` when that exact code is already taken in this tenant, which
 * is the allocation loop's signal to generate another one. The match is
 * narrowed to the `(tenant_id, code)` unique index by name: a violation of any
 * other constraint — or any other failure whatsoever — propagates untouched,
 * so a real bug can never be retried away as a code collision.
 */
async function insertCardRowIfCodeIsFree(
  cardTypeId: string,
  tenantId: string,
  code: string,
): Promise<Card | null> {
  try {
    return await insertCardRow(cardTypeId, tenantId, code);
  } catch (error) {
    if (isUniqueViolation(error, CARDS_TENANT_CODE_UNIQUE)) return null;
    throw error;
  }
}
 ...
 * ## Code handling
 *
 * - **Given a code** — used verbatim, whatever its charset (it is usually
 *   printed on a pre-existing physical card). Checked for uniqueness up front
 *   and rejected with `DuplicateCodeError`, exactly as it always has been.
 * - **No code** (absent, or blank) — the card gets a random purely-numeric code
 *   allocated by `withGeneratedCode`: generate, attempt the insert, and on a
 *   `(tenant_id, code)` collision generate another. There is deliberately **no**
 *   pre-check SELECT on that path — the Neon HTTP driver has no interactive
 *   transactions, so a check-then-insert would be racy; the unique index is the
 *   arbiter. See `src/lib/card-codes/` for the adaptive length policy.
 *
 * Only the card row itself is inside the retry loop. Field values are written
 * afterwards, once a code is secured, so a collision can never leave orphans.
 *
 * @param cardTypeId - Card type UUID.
 * @param tenantId   - Tenant UUID.
 * @param code       - Client-facing card code (unique per tenant). Omit, or
 *                     pass an empty value, to have one generated.
 * @param values     - Field values keyed by field definition ID.
 * @returns The created card with enriched field values.
 * @throws {DuplicateCodeError} If an explicitly given code already exists for this tenant.
 * @throws {ValidationError} On field validation failures.
 * @throws {CardCodeExhaustedError} If no generated code could be placed.
 */
export async function createCard(
  ...
  // A blank code is "no code", not a code made of spaces. Anything else is
  // taken verbatim — never trimmed, never reformatted.
  const manualCode = code != null && code.trim() !== "" ? code : null;

  // Check code uniqueness (manual codes only).
  if (manualCode !== null) {
    const [existing] = await db
      .select({ id: cards.id })
      .from(cards)
      .where(and(eq(cards.tenantId, tenantId), eq(cards.code, manualCode)))
      .limit(1);

    if (existing) {
      throw new DuplicateCodeError(manualCode, tenantId);
    }
  }
  ...
  // Validate values before any insert, so a rejected card leaves no row behind.
  validateFieldValues(values, defs);

  // Insert card.
  const card =
    manualCode !== null
      ? await insertCardRow(cardTypeId, tenantId, manualCode)
      : await withGeneratedCode((candidate) =>
          insertCardRowIfCodeIsFree(cardTypeId, tenantId, candidate),
        );
