...
**Last updated**: 2026-09-08 · **Last feature**: feed and panel photos carry a `?v=` version token and cache for days; photo keys no longer cross to the client
...
- `src/components/dashboard/DashboardSearchBar.tsx` — **Operational scan input**: manual code entry + external reader. Calls `onScan(code)` → `executeScanWithAutoActionsAction` in parent. Keeps focus at all times (never `disabled`/`readOnly`) and QUEUES codes read while the dashboard is busy, draining one per return to idle — `DashboardView` passes `isBlocked` to hold that drain while a modal, resume, or manual action owns `activeCard`. See `modules/scanning.md`. Visually the focal point of the page.
...
Payloads are stripped of `photo` object keys at the Server Action boundary (`loadClientSnapshots`), alongside `stripScanResultPhotos` — which redacts the result card's own photo values to presence flags (it replaced `signScanResultPhotos` in ADR `2026-09-08-card-payload-photo-key-redaction.md`).
...
- Serves: feed thumbnails and the `ActiveCardZone` panel photo cell both via `/api/photos/cards/[code]` (`cardPhotoRoute` in `@/lib/storage/photo-routes`, `PhotoRenderer` in `src/components/shared/`), which signs per request. Both pass the **card's** `updatedAt` so the URL carries `?v=` and a replaced photo is a cache miss — ADR `2026-09-08-photo-cache-version-token.md`. No dashboard payload carries a photo key or a signed URL any more: the scan, resume and refresh actions all run `stripCardPhotoKeys`, so a photo value here means "there is a photo" and nothing else. `signCardPhotos` survives only for the external API route and the card-detail server page. ADRs `2026-08-25-active-card-zone-stable-photo-route.md`, `2026-09-08-card-payload-photo-key-redaction.md`.
...
- 2026-09-08 — **Feed photos gained a version token, and a thumbnail bug it would otherwise have caused.** `getActivityFeed` and `feed-entries.ts` both build `cardPhotoUrl` with the card's `updatedAt` (one extra column on a query that already joined `cards`), so an edited photo refreshes despite the route's new multi-day cache. In the same pass the scan/resume/refresh actions stopped shipping photo keys and signed URLs and now redact to a boolean — which broke `makeEntry`'s `hasPhoto`, written as `typeof f.value === "string"` back when the value was always a signed URL: every feed thumbnail silently vanished until the predicate became a truthiness test. `ActiveCardZone` passes `activeCard.updatedAt` from both `SummaryGrid` branches; neither needed a lookup map, since both already hold the card. ADRs `2026-09-08-photo-cache-version-token.md`, `2026-09-08-card-payload-photo-key-redaction.md`.
...
`2026-07-17-card-lifecycle-scan-behaviour.md`; the `ActiveCardZone` signed-photo
fix (2026-08-25) in `2026-08-25-active-card-zone-stable-photo-route.md`; and the
feed-limit-counts-groups fix (2026-08-25) in
`2026-08-25-feed-limit-counts-groups.md`._
