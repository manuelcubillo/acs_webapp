...
**Last updated**: 2026-08-25 · **Last feature**: `ActiveCardZone`'s photo cell moved to the stable photo route (fixed: photo disappeared after a manual action)
...
- 2026-08-25 — Fixed: `ActiveCardZone`'s photo cell went blank after executing a manual action. Root cause: the panel read the photo field's raw value as a ready-to-use `<img src>`, which is only a signed URL when `activeCard` came from the scan/resume paths (`signCardPhotos`) — `executeAndRefresh`'s plain `getCardByCodeAction` refetch never signs it. `SummaryCell` now renders the field through `PhotoRenderer` (moved to `src/components/shared/`, gained an optional `className` for the panel's tall two-row cell) in stable-route mode (`cardCode` + `fieldDefinitionId`), matching every other photo surface. ADR `2026-08-25-active-card-zone-stable-photo-route.md`.
