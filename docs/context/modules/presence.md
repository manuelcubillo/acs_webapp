...
**Last updated**: 2026-08-25 · **Last feature**: `PresenceControl` (Entrada/Salida segments) and direction labels across feed, history and export
...
- `src/app/(dashboard)/presence/PresenceClient.tsx` — Occupancy view: total, per-card-type groups, client-side search, per-row exit switch, Refrescar + "Actualizado HH:MM".
- `src/lib/dal/presence.ts` — `getPresenceOccupants(tenantId)`, `tenantHasPresenceEnabled(tenantId)`.
- `src/lib/actions/presence.ts` — `getPresenceOccupantsAction` (OPERATOR+). Read-only; flipping goes through `executeActionAction`.
...
- `src/lib/db/constants.ts` — `SYSTEM_USER_ID`. Seeded, referenced by nothing yet; the auto-close job is its first consumer.
...
- Writes through: `actions` (`executeAction` — presence never writes `field_values` directly).
