...
- `src/lib/actions/card-types.ts` — Server Actions for create/update, plus `setPresenceControlAction(cardTypeId, enabled)` (MASTER). Lifecycle moved out (see below).
...
- `card_types(id, tenant_id, name, description, status, archived_at, archived_by, status_before_archive, presence_field_definition_id, timestamps)`

`presence_field_definition_id` (nullable FK → `field_definitions`, `ON DELETE SET NULL`) designates the boolean field holding "inside the facility". NULL = this card type does not participate in presence control. **The designation IS the state** — the wizard's checkbox reads `!== null` and there is nothing else to consult.
...
3. Submit runs Server Actions sequentially: create `card_type` → create fields → create actions → create scan validations → `setPresenceControlAction` (last, so the provisioned system field lands after the user's fields).
...
⚠️ The loader drops `is_system` rows **before** the tempId mapping (`excludeSystemFields` / `excludeSystemActions`), so they never enter the draft state. If one did, the diffing would treat it as user-owned: reordering could move it and removing it would deactivate the presence field out from under the feature.

`presenceEnabled` lives on `basicInfo` and is submitted **unconditionally** via `setPresenceControlAction(id, desired)` — provisioning is idempotent both ways, so the wizard never has to know which transition it is.
...
- **New CardType-level setting** → add column to `card_types`, extend `BasicInfoStep` (and `BasicInfo` in `useCardTypeWizard`), extend DAL and Server Actions. Presence control is the worked example end to end.
