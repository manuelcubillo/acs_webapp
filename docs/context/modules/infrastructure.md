...
**Last updated**: 2026-09-08 · **Last feature**: photo URLs carry a card-level version token, so the route and its bytes cache for days instead of minutes
...
- `src/lib/dal/photo-urls.ts` — Server-only helpers. `stripCardPhotoKeys` (single card) / `stripCardListPhotoKeys` (a map over it) replace a photo key — in both `value` and `raw.value_text` — with a boolean presence flag; this is what **every** action returning a card to a client component applies. `signCardPhotos` / `buildPhotoReadUrlMap` do the opposite, and survive only for the two server-side consumers that need a real URL: the card-design preview renderer and the external API. ADR `2026-09-08-card-payload-photo-key-redaction.md`.
...
- `src/lib/storage/s3-base.ts` — Shared S3-compatible class (presigned PUT/GET, head, delete, prefix delete). `getReadUrl` takes an optional `downloadFilename` → signed `ResponseContentDisposition`, and an optional `responseCacheControl` → signed `ResponseCacheControl` (both are part of the signature, so neither can be edited onto a minted URL). `endpoint` is optional and **omitted** from the `S3Client` config when unset — passing the key at all disables the SDK's regional endpoint resolver.
...
- `src/lib/storage/read.ts` — `signPhotoForRead`, `signPhotoForReadOptional`, `signPhotosForRead`, `signPhotoForDownload` (attachment). **15-min TTL** — every caller but the photo route hands the signed URL straight to a client or an external caller, where it is a bearer token, so the short default is the leak bound. `signPhotoForRead` takes an optional trailing `{ responseCacheControl }`, worth setting only for a caller whose URL is stable enough to be re-requested.
- `src/lib/storage/photo-routes.ts` — `cardPhotoRoute(code, { fieldDefinitionId?, download?, updatedAt? })`, the single builder for the photo route's URL and query. `updatedAt` (the **card's**, not the field's) serialises to `?v=<epoch seconds>` and is what busts the browser cache when a photo is replaced — see ADR `2026-09-08-photo-cache-version-token.md`. Dependency-free on purpose: imported by both the DAL and client components (`PhotoRenderer` builds its `<img src>` and download href with it).
- `src/app/api/photos/cards/[code]/route.ts` — Session-authed (OPERATOR+) card photo: 302 → signed URL minted per request. Stable per card, so it neither expires client-side nor busts the browser cache. Signs with a **7-day** TTL (`READ_URL_TTL_SECONDS`, the SigV4 ceiling — `@smithy/signature-v4` rejects anything above it), caches the redirect **6 days** (`REDIRECT_MAX_AGE_SECONDS`, which MUST stay below the signature TTL or a replayed redirect 403s), and asks the store for `private, max-age=604800, immutable` on the bytes (inline reads only, never the download). Both constants move together. Optional `?field=<fieldDefinitionId>` picks a specific photo field; `?download` returns an attachment named `<code>_<fieldName>_<random>.<ext>` (default: primary photo, inline). `?v=` is deliberately **not read** — the route always resolves the live key. ADRs `2026-07-17-stable-photo-routes.md`, `2026-07-19-webcam-capture-and-crop.md`, `2026-09-08-photo-cache-version-token.md`.
...
- 2026-09-08 — **Photo caching decoupled from photo freshness.** `cardPhotoRoute` gained `updatedAt` → `?v=<epoch s>`, sourced from `cards.updated_at`; an edit is a new URL, so freshness no longer depends on how long anything is cached. That freed the route to sign for 7 days (the SigV4 ceiling, confirmed against the installed `@smithy/signature-v4`: 604800 accepted, 604801 rejected), cache its redirect for 6, and ask the store for `immutable` bytes via a new `responseCacheControl` on `ReadUrlOptions` — previously the object response carried **no** cache directive at all and fell back to undefined heuristic caching. The shared `DEFAULT_READ_TTL` / `DEFAULT_TTL_SECONDS` stay at 15 min on purpose: every other caller ships the signed URL as a bearer token. Fixed on the way: the route set its `Cache-Control` on downloads too while signing them with the 15-min default, which the new `max-age` would have turned into a live 403. Card-level token, not the photo field's own — the alternative duplicates `route.ts`'s "which photo field is primary" rule into two DAL files and fails silently when they drift. ADR `2026-09-08-photo-cache-version-token.md`.
- 2026-09-08 — Photo key redaction unified: `stripCardPhotoKeys` is the single helper every action returning a card to a client applies, and `stripCardListPhotoKeys` is now a map over it. Closes a leak in `getCardByCodeAction`, which shipped the raw object key to the browser. ADR `2026-09-08-card-payload-photo-key-redaction.md`.
...
_Pruned to the 5-entry cap: the presence-control migrations (2026-08-24) are
described in `2026-08-24-presence-control.md`, and the card-design export-size
columns (2026-08-06) in `2026-08-06-card-design-export-size.md`._
