...
**Last updated**: 2026-08-30 · **Last feature**: Invitation refund by visit duration — a 5-hour window replaces the half-day slot
...
- `src/lib/action-strategies/invitation-strategy.ts` — The one live custom strategy (`scan_strategy = 'invitation'`). Duration-based guest entry/exit accounting over two number fields, plus a per-card boolean that opts a card out of it (R4). Exported pure functions (`resolveLocalDate`, `decideEntry`, `decideExit`, `selectSettlements`, `countSettlements`, `earliestExecutedAt`, `buildRefundCounters`, `toFlag`, …) with I/O at the edges; `INVITATION_CONFIG` at the top holds the five tenant UUIDs, `REFUND_WINDOW_HOURS` the threshold. ADRs `2026-08-30-invitation-duration-refund.md`, `2026-08-27-invitation-accounting.md`, `2026-08-29-invitation-purchase-mode.md`.
...
| …strategy result | whatever a `TenantActionStrategy` returns — currently `invitationSettlement`, `invitationDate`, `invitationMode` |
...
**`invitationSettlement`** (`full_spent | purchase_spent | half_spent | half_refunded | none`) is the first **strategy-owned** metadata key: written by `InvitationActionStrategy` on every guest entry/exit and read back by it to cap refunds. It is the settlement record of record — for that tenant `before_value` / `after_value` are NOT sufficient, because three of the five settlements leave the target field unchanged. `purchase_spent` and `full_spent` differ *only* in refundability: R2 counts the latter, never the former, which is what keeps an R4 entry unrefundable even after the flag is cleared. `invitationMode` (`purchase | duration | slot`) rides alongside as a diagnostic — an R4 exit settles as `none`, otherwise indistinguishable in the log from a duration exit that hit its cap. `slot` is **historical only**: rows written before 2026-08-30, plus the legacy rows `resync.ts` reconstructs, which were settled by neither current rule. Neither key is declared in `metadata-keys.ts`: only the strategy reads them. `invitationSlot` was removed on 2026-08-30 with the half-day rule; the refund window is derivable as `executed_at − 5h`. ADRs `2026-08-30-invitation-duration-refund.md`, `2026-08-27-invitation-accounting.md`, `2026-08-29-invitation-purchase-mode.md`.
...
The only live custom strategy. `GUEST_ENTRY` spends a `HALF_INVITATION` credit if one exists (not refundable), otherwise an `INVITATIONS` unit (refundable). `GUEST_EXIT` returns one half-credit while `refunded < spent`, where a visit is refundable when it lasted **at most 5 hours** (`REFUND_WINDOW_HOURS`, inclusive, compared in milliseconds) **and** entry and exit fall on the same `Europe/Madrid` calendar day. So: `spent` = `full_spent` entries today with `executedAt >= now − 5h`; `refunded` = `half_refunded` exits today since **the oldest counted entry** — not since the same cutoff, because a refund granted before that entry existed cannot belong to any entry being counted, and charging it against one would deny a fresh guest their credit. Both counts come from `invitationSettlement` markers on prior `log_type='action'` rows — there is still no entry↔exit pairing key, which is what keeps the rule order-independent and therefore correct when a member walks several guests in at once. ADR `2026-08-30-invitation-duration-refund.md` (supersedes the half-day slot rule).
...
- 2026-08-30 — **Invitation refund by visit duration.** The half-day slot is gone: `GUEST_EXIT` now refunds when the visit lasted **at most 5 hours** (`REFUND_WINDOW_HOURS`, inclusive, milliseconds) and entry and exit share a `Europe/Madrid` day. The counter model survives — `spent` = `full_spent` entries inside the window, `refunded` = `half_refunded` exits **since the oldest counted entry** (not since the same cutoff, or a stale refund would swallow a fresh entry's) — which is what keeps it order-independent for several guests on one card. `resolveSlot` / `InvitationSlot` / `AFTERNOON_START_HOUR` / `LocalMoment` / `invitationSlot` deleted; `resolveLocalMoment` → `resolveLocalDate`; `SlotCounters` → `RefundCounters`; new `selectSettlements`, `earliestExecutedAt`, `buildRefundCounters`. `invitationMode` writes `duration`, keeping `slot` for pre-change and legacy-import rows. No schema change, no migration, no backfill, no context change; two production files (`resync.ts` follows the renames). 49 → 60 unit tests. ADR `2026-08-30-invitation-duration-refund.md`.
- 2026-08-29 — **Invitation purchase mode (R4).** A per-card boolean, `compra_invitaciones`, opts a card out of half-day slot accounting: `GUEST_ENTRY` still spends an owned half-credit first but marks a full-invitation entry `purchase_spent`, and `GUEST_EXIT` refunds nothing and never reads the history. The entry arithmetic is unchanged — the whole delta is on exit. The new marker exists so R2's refundable count excludes those entries permanently, surviving a flag cleared mid-day; `decideExit` took `SlotCounters | null` (since renamed `RefundCounters`) rather than fabricated zero counters. New `invitationMode` diagnostic key. No schema change, no migration, no context change; one production file. 33 → 49 unit tests. ADR `2026-08-29-invitation-purchase-mode.md`.
...
_Pruned to the 5-entry cap: the initial 2026-04-19 extraction note is dropped, the phase-2 lifecycle gate (2026-07-17) is described in `2026-07-17-card-lifecycle-scan-behaviour.md`, the 2026-08-24 `toggle` / `is_operator_visible` entry in `2026-08-24-presence-control.md`, and the 2026-08-25 `metadataExtra` / `metadata-keys.ts` entry in `2026-08-25-feed-grouping-and-scan-correlation.md` (the `metadataExtra` contract itself is documented above, under the `metadata` contract table)._
