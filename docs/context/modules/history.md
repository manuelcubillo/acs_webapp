...
**Last updated**: 2026-09-08 · **Last feature**: field filters and the Detail column understand a multi-select value
...
`select` fields reuse `equals_text`. Since 2026-09-08 a select stores ONE option in `value_text` and SEVERAL in `value_json`, so the text operators go through `fieldValueTextIlike` / `fieldValueTextEquals` (`src/lib/dal/field-value-sql.ts`), which match both columns; on a multi-select `equals_text` therefore reads as CONTAINMENT ("cards whose Zona includes Lisboa"). Their value input is a dropdown of the field's configured options rather than a free-text box. See `modules/fields.md` and ADR `2026-09-08-multi-select-storage.md`.
...
5. Client renders `HistoryTable`. Pagination controls call `getActionHistoryAction` on page change. `HistoryTableRow` renders a `photo` summary field as a thumbnail addressed by `cardPhotoRoute(cardCode, { fieldDefinitionId, updatedAt: entry.cardUpdatedAt })`, the same stable route the dashboard feed uses; every other type is formatted as text. `cardUpdatedAt` is **live**, like `cardCode` and unlike the snapshot-projected values beside it: the thumbnail always resolves the card's photo as it stands today, so freezing the token to the event would pin a stale image. ADR `2026-09-08-photo-cache-version-token.md`.
...
- **New field filter operator** → add to `FieldFilterOperatorSchema` (Zod), `buildFieldFilterSQL`, and `HistoryFieldFilters` UI. A TEXT operator must go through `src/lib/dal/field-value-sql.ts`, or it will silently miss every multi-select value.
...
- 2026-09-08 — The Resumen photo thumbnail gained a cache-busting `?v=`. `ActionHistoryEntry` carries a new `cardUpdatedAt`, selected off the `cards` join both queries already had and threaded through the intermediate `RawRow` type, so an edited photo appears despite the photo route now caching for days. Live, not snapshot-frozen — same rule as `cardCode`, because the thumbnail resolves today's object. ADR `2026-09-08-photo-cache-version-token.md`.
- 2026-09-08 — Multi-select support reached this module in three places: the text field filters now match inside `value_json` via the shared `field-value-sql.ts` predicate (a filter reading only `value_text` returned a WRONG result set, not an error), `formatValue` renders an array as a comma-separated list in the Detail column and the CSV, and `diffSnapshots` compares arrays by value — `===` on two arrays would have reported every scan of a multi-select card as a change. ADR `2026-09-08-multi-select-storage.md`.
...
_Pruned to the 5-entry cap: the select-field filter fix and the `/cards` URL-state extraction (both 2026-08-02) are described in `modules/fields.md` → Select options and in `2026-08-02-card-list-url-state-and-return.md`; the field-filter-without-a-card-type fix (2026-08-15) is covered by `src/lib/dal/__tests__/history-field-filters.integration.test.ts`._
